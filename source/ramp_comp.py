...
MeanFlow2.PlanarData.loc[ind0, 'walldist'] = 0.0
...
ind = np.argmin(np.abs(Cf2_up[:-50]))
...
# %% skin friction
fig2, ax2 = plt.subplots(figsize=(15*cm2in, 5*cm2in), dpi=300)
...
ax2.set_ylim([-1.0e-3, 6.0e-3])
ax2.set_xticks(np.arange(-160, 80.0 + 40.0, 40))
...
ax2.legend(["case 1", "case 2"], loc='upper left', fontsize=nsize-2, framealpha=0.4)
#            ncols=2, columnspacing=0.6, )
plt.tick_params(labelsize=nsize)
plt.savefig(pathC + "Cf_comp3.svg", bbox_inches="tight", pad_inches=0.1)
plt.show()

xx = np.hstack((xwall_up, xwall_down))
ff = np.hstack((Cf2_up_fit, Cf2_down_fit))
df = pd.DataFrame(np.vstack((xx, ff)).T, columns=['x', 'Cf'])
df.to_csv(pathM2 + 'Cf_fit.dat', index=False, float_format='%1.8e')
# %% Cp
Ma = 6.0
fa = 1 / ramp_wall2["p"][0]
b, a = signal.butter(6, Wn=1 / 12, fs=1 / 0.25)
...
# %%
fig3, ax3 = plt.subplots(figsize=(15*cm2in, 5*cm2in), dpi=300)
# ax3 = fig.add_subplot(212)
ax3.plot(ramp_wall2['x'], ramp_wall2["p"] * fa, "b", linewidth=1.5)
ax3.plot(ramp_wall['x'], ramp_wall["p"] * fa, "r", linewidth=1.5)
...
ax3.set_ylim([0, 7])
ax3.set_xticks(np.arange(-160, 80.0 + 40.0, 40))
...
ax3.legend(["case 1", "case 2"], loc='upper left', fontsize=nsize-2, framealpha=0.4)
plt.tick_params(labelsize=nsize)
plt.savefig(pathC + "Cp_comp3.svg", bbox_inches="tight", pad_inches=0.1)
plt.show()

xx = ramp_wall['x']
ff = ramp_wall["p"] * fa
df = pd.DataFrame(np.vstack((xx, ff)).T, columns=['x', 'Cp'])
df.to_csv(pathM + 'Cp.dat', index=False, float_format='%1.8e')
# %% Cp certificate
p_ref = pd.read_csv(pathC + "p-x.txt", sep=' ', skipinitialspace=True)
fig3, ax3 = plt.subplots(figsize=(7*cm2in, 6*cm2in), dpi=300)
xarr = ramp_wall2["x"] / (xline2_re - xline2_se)
ax3.plot(xarr, Cp2_fit * fa, "b", linewidth=1.5)
# xarr = ramp_wall["x"] / (xline_re - xline_se)
# ax3.plot(xarr, Cp_fit * fa, "b", linewidth=1.5)
ind = 8
...
ax3.grid(visible=True, which="both", linestyle=":")
plt.tick_params(labelsize=nsize)
plt.savefig(pathC + "Cp_ref2.svg", bbox_inches="tight", pad_inches=0.1)
plt.show()
# %% Stanton number
firstval = 0.015625
T_wall = 3.35
...
ax2.plot(xwall, np.abs(Cs2_fit), "b", linewidth=1.5)
ax2.plot(xwall, np.abs(Cs_fit), "r", linewidth=1.5)

ax2.set_xlabel(r"$x/l_r$", fontsize=tsize)
ax2.set_ylabel(r"$\langle C_s \rangle$", fontsize=tsize)
ax2.set_xlim([-160.0, 80])
ax2.set_ylim([-0.5e-3, 6.5e-3])
ax2.set_xticks(np.arange(-160, 80.0 + 40.0, 40))
ax2.ticklabel_format(axis="y", style="sci", scilimits=(-2, 2))
ax2.axvline(x=xline_se, color="r", linestyle=":", linewidth=1.0)
ax2.axvline(x=xline2_se, color="b", linestyle=":", linewidth=1.0)
...
ax2.legend(["case 1", "case 2"], loc='upper left', fontsize=nsize-2,
           columnspacing=0.6, framealpha=0.4)
plt.tick_params(labelsize=nsize)
plt.savefig(pathC + "Cs_comp3.svg", bbox_inches="tight", pad_inches=0.1)
plt.show()

xx = xwall
ff = np.abs(Cs2_fit)
df = pd.DataFrame(np.vstack((xx, ff)).T, columns=['x', 'Cs'])
df.to_csv(pathM2 + 'Cs_fit.dat', index=False, float_format='%1.8e')
# %% growth rate
varn = "<u`u`>"
if varn == "<u`u`>":
    savenm = "MaxRMS_u"
...
ax.plot(ramp_max["x"], np.sqrt(ramp_max[varn]), "r")
ax.plot(ramp_max2["x"], np.sqrt(ramp_max2[varn]), "b")
ax.set_xlim([-180.0, 80])
ax.set_ylim([1e-4, 3e-1])
ax.set_xticks(np.linspace(-180.0, 80.0, 14))
ax.set_yscale("log")
...
plt.savefig(pathC + "MaxFluc" + savenm + "3.svg", bbox_inches="tight")
plt.show()