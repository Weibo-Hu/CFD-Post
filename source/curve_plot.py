...
# %%
WallFlow = MeanFlow.PlanarData[np.round(MeanFlow.PlanarData["y"], 6) == 3.3333e-02]
FirstLev = WallFlow[["x", "y"]]
FirstLev.to_csv(path + "FirstLev.dat", index=False, float_format="%9.6f")
# %% wall buondary
# va.wall_line(MeanFlow.PlanarData, path, mask=corner, val=0.0333333)
MeanFlow.copy_meanval()
wavy = pd.read_csv(pathM + "FirstLev.dat", delimiter=",", skipinitialspace=True)
xx = np.arange(0.0, 400.0, 0.25)
wavy = wavy[wavy["x"].isin(xx)]
wavy["p"] = griddata(
    (MeanFlow.x, MeanFlow.y), MeanFlow.p, (wavy.x, wavy.y), method="cubic"
)
wavy["T"] = griddata(
    (MeanFlow.x, MeanFlow.y), MeanFlow.T, (wavy.x, wavy.y), method="cubic"
)
wavy["u"] = griddata(
    (MeanFlow.x, MeanFlow.y), MeanFlow.u, (wavy.x, wavy.y), method="cubic"
)
wavy["v"] = griddata(
    (MeanFlow.x, MeanFlow.y), MeanFlow.v, (wavy.x, wavy.y), method="cubic"
)
mu = va.viscosity(25600, wavy["T"])
Cf = va.skinfriction(mu, wavy["u"], 0.0166667).values  # 0.0333333
ddx = np.diff(wavy["x"])
ddy = np.diff(wavy["y"])
...
delta_y = 0.033333 * np.abs(cos_val)
delta_x = 0.033333 * np.abs(sin_val)
Cf = (
